...
struct SwiGluAttrs : public tvm::AttrsNode<SwiGluAttrs> {
  /*
                   x
              //      \\
             //        \\
      swish(x*w1+b1) * (x*w2+b2)
   
  */
  bool gate_bias_flag;
  bool up_bias_flag;
  // Array<float> gate_weight;
  // Array<float> up_weight;

  TVM_DECLARE_ATTRS(SwiGluAttrs, "relay.attrs.SwiGluAttrs") {
    TVM_ATTR_FIELD(gate_bias_flag).set_default(false).describe(
      "feedforward0 gate matmul bias existence or inexistence"
    );
    TVM_ATTR_FIELD(up_bias_flag).set_default(false).describe(
      "feedforward0 up matmul bias existence or inexistence"
    );
    // TVM_ATTR_FIELD(gate_weight).describe("feedfoeward0 gate matmul weight");
    // TVM_ATTR_FIELD(up_weight).describe("feedforeard0 up matmul weight");
  }
};
