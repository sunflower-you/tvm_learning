...
#swiglu
@override_native_generic_func("swiglu_strategy")
def swiglu_strategy(attrs, outs, out_type, target):
    """swiglu"""
    strategy = _op.OpStrategy()
    strategy.add_implementation(
        wrap_swiglu(topi.swiglu_ffn),
        wrap_topi_schedule(topi.generic.schedule_swiglu_ffn),
        name="swiglu.generic",
    )
    return strategy

def wrap_swiglu(topi_compute):
    """Wrap swiglu style topi compute"""

    def _compute_swiglu(attrs, inputs, _):
        return [topi_compute(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], attrs.gate_bias_flag, attrs.up_bias_flag)]

    return _compute_swiglu
